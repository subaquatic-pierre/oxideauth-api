...
-- Purpose: Define multi-tenant containers ("namespaces") that isolate projects, memberships,
--   and resources under a common scope. A namespace acts as the root boundary for tenant data.
-- Notes:
--   - `name` is human-readable; `slug` is a URL-safe unique identifier (e.g. used in routing).
--   - `config` is a JSONB object for per-namespace settings (feature flags, limits, branding).
--   - `created_by` / `updated_by` are audit fields; FK constraints can be added later to avoid
--     bootstrap problems (see TODOs).
CREATE TABLE IF NOT EXISTS
  namespace (
    -- Primary key for namespace (globally unique).
    ...
    description TEXT,
    -- Config blob for per-namespace structured settings (JSON object enforced below).
